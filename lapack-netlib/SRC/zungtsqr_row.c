...
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zunrgts
qr_row.f"> */
/* > [TGZ]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zunrgts
qr_row.f"> */
/* > [ZIP]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zunrgts
qr_row.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
...
/*     its diagonal elements to one. */
    ...
    zlaset_("U", m, n, &c_b2, &c_b1, &a[a_offset], lda);
...
/* Computing MIN */
		...
		i__4 = *n - kb + 1;
		zlarfb_gett_("I", &imb, &i__4, &knb, &t[(jb_t__ + kb - 1) * 
...
/*           reference DUMMY(1,1) to B with LDDUMMY=1. */
	    ...
	    i__1 = *n - kb + 1;
	    zlarfb_gett_("N", &c__0, &i__1, &knb, &t[kb * t_dim1 + 1], ldt, &
		    a[kb + kb * a_dim1], lda, dummy, &c__1, &work[1], &knb);
	} else {
	    i__1 = mb1 - kb - knb + 1;
	    i__3 = *n - kb + 1;
	    zlarfb_gett_("N", &i__1, &i__3, &knb, &t[kb * t_dim1 + 1], ldt, &
