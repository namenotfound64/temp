...
/*        Determine both COLMAX and IMAX. */
	...
	if (f2cmax(absakk,colmax) == 0.f || sisnan_(&absakk)) {
...
/*              element in row IMAX, and ROWMAX is its absolute value */
		...
		rowmax = (r__1 = a[i__1].r, abs(r__1)) + (r__2 = r_imag(&a[
			imax + jmax * a_dim1]), abs(r__2));
		if (imax > 1) {
		    i__1 = imax - 1;
		    jmax = icamax_(&i__1, &a[imax * a_dim1 + 1], &c__1);
/* Computing MAX */
		    i__1 = jmax + imax * a_dim1;
		    r__3 = rowmax, r__4 = (r__1 = a[i__1].r, abs(r__1)) + (
			    r__2 = r_imag(&a[jmax + imax * a_dim1]), abs(r__2)
			    );
...
/*                 no interchange, use 1-by-1 pivot block */
		    ...
		    if ((r__1 = a[i__1].r, abs(r__1)) + (r__2 = r_imag(&a[
			    imax + imax * a_dim1]), abs(r__2)) >= alpha * 
			    rowmax) {
