...
/* Computing MAX */
    ...
    if (*info == 0) {
	zsytrf_aa_(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], &c_n1, 
		info);
	lwkopt_sytrf__ = (integer) work[1].r;
	zsytrs_aa_(uplo, n, nrhs, &a[a_offset], lda, &ipiv[1], &b[b_offset], 
...
/*     Compute the factorization A = U**T*T*U or A = L*T*L**T. */
    ...
    zsytrf_aa_(uplo, n, &a[a_offset], lda, &ipiv[1], &work[1], lwork, info);
    if (*info == 0) {
