...
static doublecomplex c_b15 = {0.,0.};
static doublecomplex c_b23 = {-1.,0.};
...
/*  ===================================================================== */
/* Subroutine */ void zhetrf_aa_2stage_(char *uplo, integer *n, doublecomplex 
	...
	integer *ipiv2, doublecomplex *work, integer *lwork, integer *info)
{
    ...
    extern void zlacpy_(char *, integer *, integer *, doublecomplex *,
	    ...
	    doublecomplex *, integer *), zhegst_(integer *, char *, 
...
/*                  H(I,J) = T(I,I)*U(I,J) + T(I+1,I)*U(I+1,J) */
		    ...
		    i__3 = ldtb - 1;
		    zgemm_("NoTranspose", "NoTranspose", &nb, &kb, &jb, &
			    c_b14, &tb[td + 1 + i__ * nb * ldtb], &i__3, &a[(
			    i__ - 1) * nb + 1 + (j * nb + 1) * a_dim1], lda, &
			    c_b15, &work[i__ * nb + 1], n);
		} else {
/*                 H(I,J) = T(I,I-1)*U(I-1,J) + T(I,I)*U(I,J) + T(I,I+1)*U(I+1,J) */
		    ...
		    i__3 = ldtb - 1;
		    zgemm_("NoTranspose", "NoTranspose", &nb, &kb, &jb, &
			    c_b14, &tb[td + nb + 1 + (i__ - 1) * nb * ldtb], &
			    i__3, &a[(i__ - 2) * nb + 1 + (j * nb + 1) * 
			    a_dim1], lda, &c_b15, &work[i__ * nb + 1], n);
		}
	    }
...
/*              T(J,J) = U(1:J,J)'*H(1:J) */
		...
		zgemm_("Conjugate transpose", "NoTranspose", &kb, &kb, &i__2, 
			&c_b23, &a[(j * nb + 1) * a_dim1 + 1], lda, &work[nb 
			+ 1], n, &c_b14, &tb[td + 1 + j * nb * ldtb], &i__3);
/*              T(J,J) += U(J,J)'*T(J,J-1)*U(J-1,J) */
		i__2 = ldtb - 1;
		zgemm_("Conjugate transpose", "NoTranspose", &kb, &nb, &kb, &
			c_b14, &a[(j - 1) * nb + 1 + (j * nb + 1) * a_dim1], 
			lda, &tb[td + nb + 1 + (j - 1) * nb * ldtb], &i__2, &
			c_b15, &work[1], n);
		i__2 = ldtb - 1;
		zgemm_("NoTranspose", "NoTranspose", &kb, &kb, &nb, &c_b23, &
			work[1], n, &a[(j - 2) * nb + 1 + (j * nb + 1) * 
			a_dim1], lda, &c_b14, &tb[td + 1 + j * nb * ldtb], &
			i__2);
	    }
	    if (j > 0) {
		i__2 = ldtb - 1;
...
/*                 Compute H(J,J) */
			...
			zgemm_("NoTranspose", "NoTranspose", &kb, &kb, &kb, &
				c_b14, &tb[td + 1 + j * nb * ldtb], &i__2, &a[
				(j - 1) * nb + 1 + (j * nb + 1) * a_dim1], 
				lda, &c_b15, &work[j * nb + 1], n);
		    } else {
			i__2 = nb + kb;
			i__3 = ldtb - 1;
			zgemm_("NoTranspose", "NoTranspose", &kb, &kb, &i__2, 
				&c_b14, &tb[td + nb + 1 + (j - 1) * nb * ldtb]
				, &i__3, &a[(j - 2) * nb + 1 + (j * nb + 1) * 
				a_dim1], lda, &c_b15, &work[j * nb + 1], n);
		    }
...
/*                 Update with the previous column */
		    ...
		    zgemm_("Conjugate transpose", "NoTranspose", &nb, &i__2, &
			    i__3, &c_b23, &work[nb + 1], n, &a[((j + 1) * nb 
			    + 1) * a_dim1 + 1], lda, &c_b14, &a[j * nb + 1 + (
			    (j + 1) * nb + 1) * a_dim1], lda);
		}
...
/* Computing MIN */
		...
		kb = f2cmin(i__2,i__3);
		i__2 = ldtb - 1;
		zlaset_("Full", &kb, &nb, &c_b15, &c_b15, &tb[td + nb + 1 + j 
			* nb * ldtb], &i__2);
		i__2 = ldtb - 1;
		...
		if (j > 0) {
		    i__2 = ldtb - 1;
		    ztrsm_("R", "U", "N", "U", &kb, &nb, &c_b14, &a[(j - 1) * 
...
/*              updates */
		...
		zlaset_("Lower", &kb, &nb, &c_b15, &c_b14, &a[j * nb + 1 + ((
			j + 1) * nb + 1) * a_dim1], lda);
...
/*                    > Swap A(I1+1:M, I1) with A(I2, I1+1:M) */
			if (i2 > i1 + 1) {
			    i__3 = i2 - i1 - 1;
			    zswap_(&i__3, &a[i1 + (i1 + 1) * a_dim1], lda, &a[
				    i1 + 1 + i2 * a_dim1], &c__1);
			    i__3 = i2 - i1 - 1;
...
/*                    > Swap A(I2+1:M, I1) with A(I2+1:M, I2) */
			if (i2 < *n) {
			    i__3 = *n - i2;
			    zswap_(&i__3, &a[i1 + (i2 + 1) * a_dim1], lda, &a[
				    i2 + (i2 + 1) * a_dim1], lda);
			}
...
/*                  H(I,J) = T(I,I)*L(J,I)' + T(I+1,I)'*L(J,I+1)' */
		    ...
		    zgemm_("NoTranspose", "Conjugate transpose", &nb, &kb, &
			    jb, &c_b14, &tb[td + 1 + i__ * nb * ldtb], &i__3, 
			    &a[j * nb + 1 + ((i__ - 1) * nb + 1) * a_dim1], 
			    lda, &c_b15, &work[i__ * nb + 1], n);
		} else {
...
/*              T(J,J) = L(J,1:J)*H(1:J) */
		i__2 = (j - 1) * nb;
		i__3 = ldtb - 1;
		zgemm_("NoTranspose", "NoTranspose", &kb, &kb, &i__2, &c_b23, 
			&a[j * nb + 1 + a_dim1], lda, &work[nb + 1], n, &
			c_b14, &tb[td + 1 + j * nb * ldtb], &i__3);
/*              T(J,J) += L(J,J)*T(J,J-1)*L(J,J-1)' */
		i__2 = ldtb - 1;
		zgemm_("NoTranspose", "NoTranspose", &kb, &nb, &kb, &c_b14, &
			a[j * nb + 1 + ((j - 1) * nb + 1) * a_dim1], lda, &tb[
			td + nb + 1 + (j - 1) * nb * ldtb], &i__2, &c_b15, &
			work[1], n);
		i__2 = ldtb - 1;
		zgemm_("NoTranspose", "Conjugate transpose", &kb, &kb, &nb, &
			c_b23, &work[1], n, &a[j * nb + 1 + ((j - 2) * nb + 1)
			 * a_dim1], lda, &c_b14, &tb[td + 1 + j * nb * ldtb], 
			&i__2);
	    }
	    if (j > 0) {
...
/*                 Compute H(J,J) */
			...
			i__3 = ldtb - 1;
			zgemm_("NoTranspose", "Conjugate transpose", &kb, &kb,
				 &i__2, &c_b14, &tb[td + nb + 1 + (j - 1) * 
				nb * ldtb], &i__3, &a[j * nb + 1 + ((j - 2) * 
				nb + 1) * a_dim1], lda, &c_b15, &work[j * nb 
				+ 1], n);
		    }
...
/*                 Update with the previous column */
		    ...
		    zgemm_("NoTranspose", "NoTranspose", &i__2, &nb, &i__3, &
			    c_b23, &a[(j + 1) * nb + 1 + a_dim1], lda, &work[
			    nb + 1], n, &c_b14, &a[(j + 1) * nb + 1 + (j * nb 
			    + 1) * a_dim1], lda);
		}
...
/* Computing MIN */
		...
		if (j > 0) {
		    i__2 = ldtb - 1;
		    ztrsm_("R", "L", "C", "U", &kb, &nb, &c_b14, &a[j * nb + 
...
/*              updates */
		...
		zlaset_("Upper", &kb, &nb, &c_b15, &c_b14, &a[(j + 1) * nb + 
			1 + (j * nb + 1) * a_dim1], lda);
...
/*                    > Swap A(I1+1:M, I1) with A(I2, I1+1:M) */
			if (i2 > i1 + 1) {
			    i__3 = i2 - i1 - 1;
			    zswap_(&i__3, &a[i1 + 1 + i1 * a_dim1], &c__1, &a[
				    i2 + (i1 + 1) * a_dim1], lda);
			    i__3 = i2 - i1 - 1;
...
/*                    > Swap A(I2+1:M, I1) with A(I2+1:M, I2) */
			if (i2 < *n) {
			    i__3 = *n - i2;
			    zswap_(&i__3, &a[i2 + 1 + i1 * a_dim1], &c__1, &a[
				    i2 + 1 + i2 * a_dim1], &c__1);
			}
/*                    > Swap A(I1, I1) with A(I2, I2) */
			i__3 = i1 + i1 * a_dim1;
