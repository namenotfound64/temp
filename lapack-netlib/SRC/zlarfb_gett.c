...
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zlarfb_
gett.f"> */
/* > [TGZ]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zlarfb_
gett.f"> */
/* > [ZIP]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zlarfb_
gett.f"> */
/* > [TXT]</a> */
/* > \endhtmlonly */
...
/*        into W2=WORK(1:K, 1:N-K) column-by-column. */
	...
	i__1 = *n - *k;
	for (j = 1; j <= i__1; ++j) {
	    zcopy_(k, &a[(*k + j) * a_dim1 + 1], &c__1, &work[j * work_dim1 + 
		    1], &c__1);
	}
	if (lnotident) {
...
/*           V1 stored in A1 (diagonal ones are not stored). */
	    ...
	    i__1 = *n - *k;
	    ztrmm_("L", "L", "C", "U", k, &i__1, &c_b1, &a[a_offset], lda, &
...
/*        V2 stored in B1. */
	...
	if (*m > 0) {
	    i__1 = *n - *k;
	    zgemm_("C", "N", k, &i__1, m, &c_b1, &b[b_offset], ldb, &b[(*k + 
...
/*        T is upper-triangular. */
	...
	i__1 = *n - *k;
	ztrmm_("L", "U", "N", "N", k, &i__1, &c_b1, &t[t_offset], ldt, &work[
		work_offset], ldwork);
...
/*        V2 stored in B1. */
	...
	if (*m > 0) {
	    i__1 = *n - *k;
	    z__1.r = -1., z__1.i = 0.;
	    zgemm_("N", "N", m, &i__1, k, &z__1, &b[b_offset], ldb, &work[
...
/*           V1 stored in A1 (diagonal ones are not stored). */
	    ...
	    i__1 = *n - *k;
	    ztrmm_("L", "L", "N", "U", k, &i__1, &c_b1, &a[a_offset], lda, &
		    work[work_offset], ldwork);
	}
...
/*        W1 is upper-triangular with zeroes below the diagonal. */
	...
	ztrmm_("L", "L", "C", "U", k, k, &c_b1, &a[a_offset], lda, &work[
		work_offset], ldwork);
    }
...
/*     W1 is upper-triangular with zeroes below the diagonal. */
    ...
    ztrmm_("L", "U", "N", "N", k, k, &c_b1, &t[t_offset], ldt, &work[
	    work_offset], ldwork);
...
/*     V2 = B1, W1 is upper-triangular with zeroes below the diagonal. */
    ...
    if (*m > 0) {
	z__1.r = -1., z__1.i = 0.;
	ztrmm_("R", "U", "N", "N", m, k, &z__1, &work[work_offset], ldwork, &
		b[b_offset], ldb);
    }
...
/*        and square on output. */
	...
	ztrmm_("L", "L", "N", "U", k, k, &c_b1, &a[a_offset], lda, &work[
		work_offset], ldwork);
...
/*        col1_(6)_a Compute elements of A1 below the diagonal. */
	...
	for (j = 1; j <= i__1; ++j) {
		...
		z__1.r = -work[i__4].r, z__1.i = -work[i__4].i;
		a[i__3].r = z__1.r, a[i__3].i = z__1.i;
	    }
	}
...
/*     col1_(6)_b Compute elements of A1 on and above the diagonal. */
	...
	for (i__ = 1; i__ <= i__2; ++i__) {
	    ...
	    i__4 = i__ + j * a_dim1;
	    i__5 = i__ + j * work_dim1;
	    z__1.r = a[i__4].r - work[i__5].r, z__1.i = a[i__4].i - work[i__5]
		    .i;
	    a[i__3].r = z__1.r, a[i__3].i = z__1.i;
	}
    }
