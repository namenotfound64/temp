...
/* > */
/*  ===================================================================== */
/* Subroutine */ void clatbs_(char *uplo, char *trans, char *diag, char *
...
/*                    Compute the update */
/*                       x(f2cmax(1,j-kd):j-1) := x(f2cmax(1,j-kd):j-1) - */
/*                                             x(j)* A(f2cmax(1,j-kd):j-1,j) */

/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = f2cmin(i__3,i__4);
			i__3 = j;
...
/*                 Compute the update */
/*                    x(j+1:f2cmin(j+kd,n)) := x(j+1:f2cmin(j+kd,n)) - */
/*                                          x(j) * A(j+1:f2cmin(j+kd,n),j) */
...
/* Computing MIN */
		    ...
		    i__3 = i__;
		    xmax = (r__1 = x[i__3].r, abs(r__1)) + (r__2 = r_imag(&x[
			    i__]), abs(r__2));
		}
/* L110: */
	    }
