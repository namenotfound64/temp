...
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/zunhr_c
ol.f"> */
/* > [TGZ]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/zunhr_c
ol.f"> */
/* > [ZIP]</a> */
/* > <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/zunhr_c
ol.f"> */
/* > [TXT]</a> */
/* > */
/*  Definition: */
...
/*     (1-2) Solve for V2. */
    ...
    if (*m > *n) {
	i__1 = *m - *n;
	ztrsm_("R", "U", "N", "N", &i__1, n, &c_b1, &a[a_offset], lda, &a[*n 
		+ 1 + a_dim1], lda);
    }
...
/*        column-by-column, total JNB*(JNB+1)/2 elements. */
	...
	for (j = jb; j <= i__3; ++j) {
	    i__4 = j - jbtemp1;
	    zcopy_(&i__4, &a[jb + j * a_dim1], &c__1, &t[j * t_dim1 + 1], &
		    c__1);
	}
...
/*        (2-3a) Set the elements to zero. */
	...
	for (j = jb; j <= i__3; ++j) {
		...
		i__5 = i__ + j * t_dim1;
		t[i__5].r = 0., t[i__5].i = 0.;
	    }
	}

/*        (2-3b) Perform the triangular solve. */

	ztrsm_("R", "L", "C", "U", &jnb, &jnb, &c_b1, &a[jb + jb * a_dim1], 
